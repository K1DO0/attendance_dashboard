...
const DISPLAY_TIMEZONE_LABEL = 'US Eastern Time';
...
const ATTENDANCE_HEADERS = [
  ...
  'Check In Epoch', 'Check Out Epoch'
];
...
function loginAgent(ss, data) {
  ...
  return jsonResponse(true, 'Login successful.', {
    agent: safeAgent(agent, !!openRecord, openRecord ? `Currently checked in since ${clean(openRecord.values[5])}.` : 'Ready to check in.')
  });
}
...
function checkIn(ss, data) {
  ...
  // Store the real moment as epoch milliseconds (UTC-safe).
  // Display is converted to US Eastern, but duration is calculated from epoch only.
  // This avoids timezone bugs when agents are in PH, Egypt, Pakistan, etc.
  const nowMs = Date.now();
  const recordId = 'REC-' + Utilities.getUuid().slice(0, 8).toUpperCase();
  const checkInTime = formatDateTimeFromMs(nowMs);

  attendanceSheet.appendRow([
    recordId, agent.agentId, agent.name, agent.email, agent.campaign,
    checkInTime, '', '', 'Open', checkInTime, nowMs, ''
  ]);
...
function checkOut(ss, data) {
  ...
  // Calculate using UTC-safe epoch milliseconds, not timezone-formatted strings.
  const nowMs = Date.now();
  const checkOutTime = formatDateTimeFromMs(nowMs);
  const checkInMs = getCheckInEpoch(openRecord.values);
  const totalHours = calculateDuration(checkInMs, nowMs);
  ...
  attendanceSheet.getRange(openRecord.rowNumber, 12).setValue(nowMs);
...
function getCheckInEpoch(row) {
  // Column K / index 10 stores the real check-in moment in epoch milliseconds.
  // This is the ONLY value used for accurate total-hours calculation.
  const epoch = Number(row[10]);
  if (epoch && !isNaN(epoch)) return epoch;

  // Fallback only for old open rows created before epoch storage existed.
  // New check-ins always save epoch, so timezone offset bugs will not happen on fresh rows.
  const fallback = new Date(row[5]).getTime();
  return isNaN(fallback) ? null : fallback;
}

function calculateDuration(startMs, endMs) {
  if (!startMs || !endMs) return '';
  const diffMs = Math.max(0, Number(endMs) - Number(startMs));
...
function formatDateTimeFromMs(ms) {
  return Utilities.formatDate(new Date(Number(ms)), TIMEZONE, 'yyyy-MM-dd hh:mm:ss a z');
}

function formatDateTime(date) {
  return formatDateTimeFromMs(new Date(date).getTime());
}
